...
> **Automated AI-Powered GitHub Pull Request Code Review Agent built in Python, FastAPI, and Google Gemini.**
...
- **🧠 Google Gemini Integration**: Uses Gemini 2.5 Flash to perform deep semantic code reviews. Focuses on actionable issues:
...
- **🔁 Resilient API Calls**: Google Gemini API calls are wrapped in exponential backoff retry (3 attempts) using `tenacity` for transient rate limits (429) or server errors (500, 502, 503, 504).
...
flowchart TD
    ...
    DP -->|Token-Budgeted File Chunks| RA[Review Agent - Google Gemini]
...
│   ├── test_review_agent.py      # Gemini response parser & aggregation tests
...
| :--- | :--- |
| `GEMINI_API_KEY` | Your Google Gemini API key (get one free at [aistudio.google.com/apikey](https://aistudio.google.com/apikey)) |
| `GEMINI_MODEL` | Gemini model (default: `gemini-2.5-flash`) |
| `MAX_TOKENS_PER_FILE` | Max tokens budget in prompt per file (default: `4000`) |
| `MAX_RESPONSE_TOKENS` | Max tokens for Gemini review response (default: `4096`) |
...
| `REVIEW_CONCURRENCY_LIMIT` | Max concurrent file reviews dispatched to Gemini via asyncio.gather (default: `5`) |
